...
with open('rpg_backup.json', 'r') as file_with_data:
...
class Game:
    ...
    def player_choose(self):
        ...
        cprint(f"Вы находитесь в {''.join(self.current_location_name)}\n"
               ...
               f"Уже прошло времени - {self.time_in_the_game}\n"
               f"Сила героя - {self.power}", color='green')
    ...
    def monster_attack(self, player_choose):
        ...
        spent_time_monster = float(monster_name[(monster_name.find('tm') + 2):(monster_name.find('power') - 1)])
        power_monster = int(monster_name[(monster_name.find('power') + 5):])
        try:
            if power_monster >= self.power:
                raise Exception('Сила монстра больше чем твоя')
            self.power += round(power_monster * 0.1)
            self.experience += exp
...
cprint(f"Вы завершили игру с результатом:\n"
       ...
       f" - остаток времени = {game.remaining_time}\n"
       f" - сила героя = {game.power}", color='green')
